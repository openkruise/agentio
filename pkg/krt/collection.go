...
type manyCollection[I, O any] struct {
	...
	// recomputeTotal counts transform executions for observability; it is
	// surfaced through Metadata and the debug dump so a stale or over-active
	// collection can be identified without guesswork.
	recomputeTotal atomic.Uint64
	// lastSlowLogNs rate-limits the slow-transform warning to once per
	// slowTransformLogInterval.
	lastSlowLogNs atomic.Int64
	// instrumentation gates the timing, counting, and slow warning above; when
	// off, the hot path pays none of their cost.
	instrumentation bool
}

// slowTransformThreshold bounds a single transform execution before it is
// logged; a slow transform on an incremental path is the typical first sign
// of a dependency-fanout regression.
const slowTransformThreshold = 10 * time.Millisecond

const slowTransformLogInterval = 30 * time.Second
...
func (c collectionIndex[I, O]) Lookup(key string) []O {
	...
	for k := range keys {
		...
		if !f {
			log.Error("invalid index state: object does not exist", "key", k)
			continue
		}
		res = append(res, v)
	}
...
func (h *manyCollection[I, O]) handleChangedPrimaryInputEvents(items []Event[I]) {
	...
	for idx, a := range items {
		...
		if a.Event == controllers.EventDelete {
			for oKey := range h.collectionState.mappings[iKey] {
				...
				if !f {
					h.log.Error("invalid event: deletion of non-existent object", "output_key", oKey)
					continue
				}
				e := Event[O]{
				...
				if h.log.DebugEnabled() {
					h.log.Debug("handled delete", "resource", oKey)
				}
			}
			...
			if ctx.discardUpdate {
				...
				if alreadyHasAResult || !nowHasAResult {
					h.log.Debug("discarding result", "input_key", iKey)
					continue
				}
	...
	if h.log.DebugEnabled() {
		h.log.Debug("calling handlers", "events", len(events))
	}
	h.eventHandlers.Distribute(events, !h.HasSynced())
}

func (h *manyCollection[I, O]) Metadata() Metadata {
	metadata := Metadata{"recomputeTotal": h.recomputeTotal.Load()}
	stdmaps.Copy(metadata, h.metadata)
	return metadata
}

// recordTransform accounts one transform execution and warns, at most once
// per slowTransformLogInterval, when a single execution exceeds the threshold.
func (h *manyCollection[I, O]) recordTransform(elapsed time.Duration) {
	h.recomputeTotal.Add(1)
	metrics.Default.RecordKRTTransform(elapsed > slowTransformThreshold)
	if elapsed <= slowTransformThreshold {
		return
	}
	now := time.Now().UnixNano()
	last := h.lastSlowLogNs.Load()
	if now-last < int64(slowTransformLogInterval) {
		return
	}
	if !h.lastSlowLogNs.CompareAndSwap(last, now) {
		return
	}
	h.log.Warn("transform exceeded slow threshold", "elapsed", elapsed, "threshold", slowTransformThreshold)
}
...
func newManyCollection[I, O any](
	...
	h.queue = queue.NewWithSync(func() {
		close(h.synced)
		h.log.Info("collection synced", "collection", h.name(), "uid", h.uid())
	}, h.collectionName)
...
func (h *manyCollection[I, O]) onSecondaryDependencyEvent(sourceCollection collectionUID, events []Event[any]) {
	...
	h.log.Debug("event affects objects", "event_size", len(events),
		"objects", changedInputKeys.UnsortedList())
	...
	for i := range changedInputKeys {
		...
		if iObj == nil {
			...
			h.log.Debug("parent deletion", "input_key", i)
			for oKey := range h.collectionState.mappings[i] {
				...
				if !f {
					...
					log.Error("BUG: inconsistent collection state", "input_key", i, "output_key", oKey)
					continue
				}
				e := Event[I]{
					Event: controllers.EventDelete,
