...
func configureTProxyRoutes(cfg *config.Config) error {
	if cfg.InboundPortsInclude != "" && cfg.InboundInterceptionMode == "TPROXY" {
		if err := configureTProxyRoute(cfg, cfg.InboundTProxyMark, cfg.InboundTProxyRouteTable, false); err != nil {
			return err
		}
	}
	if cfg.EnableUDPTProxy {
		return configureTProxyRoute(cfg, cfg.UDPTProxyMark, cfg.UDPTProxyRouteTable, cfg.CleanupOnly)
	}
	return nil
}

func configureTProxyRoute(cfg *config.Config, mark, routeTable string, cleanup bool) error {
	link, err := config.LinkByNameWithRetries("lo")
	if err != nil {
		return fmt.Errorf("failed to find 'lo' link: %v", err)
	}
	tproxyTable, err := strconv.Atoi(routeTable)
	if err != nil {
		return fmt.Errorf("failed to parse TPROXY route table: %v", err)
	}
	tproxyMark, err := strconv.Atoi(mark)
	if err != nil {
		return fmt.Errorf("failed to parse TPROXY mark: %v", err)
	}
	// Route captured packets using routing table ${routeTable}.
	...
	for _, family := range families {
		...
		if cleanup {
			if err := netlink.RuleDel(r); err != nil && !errors.Is(err, unix.ENOENT) {
				return fmt.Errorf("failed to delete TPROXY rule: %w", err)
			}
		} else if err := ignoreExists(netlink.RuleAdd(r)); err != nil {
			return fmt.Errorf("failed to configure netlink rule: %w", err)
		}
	}
	// In routing table ${routeTable}, create a single default rule to route all traffic to
	...
	for _, fullCIDR := range cidrs {
		...
		if cleanup {
			if err := netlink.RouteDel(route); err != nil && !errors.Is(err, unix.ESRCH) {
				return fmt.Errorf("failed to delete TPROXY route: %w", err)
			}
		} else if err := netlink.RouteAdd(route); ignoreExists(err) != nil {
